...
kdl::host::filesystem::file::file(const filesystem::path& path, file_mode mode)
    : m_path(path)
{
    if (m_path.exists()) {
...
kdl::host::filesystem::file::file(const std::string &name, const std::string &contents)
    : m_path(name)
{
...
auto kdl::host::filesystem::file::exists() const -> bool
{
    return m_path.exists() && m_length > 0 && m_raw != nullptr;
}

auto kdl::host::filesystem::file::path() const -> filesystem::path
{
    return m_path;
}

auto kdl::host::filesystem::file::string_contents() const -> const std::string&
{
...
auto kdl::host::filesystem::file::characters() const -> std::vector<char>
{
    std::vector<char> v(m_raw, m_raw + m_length);
    return std::move(v);
}

auto kdl::host::filesystem::file::set_string_contents(const std::string& contents) -> void
{
    if (m_raw) {
        delete[] m_raw;
    }
...
auto kdl::host::filesystem::file::set_characters(const std::vector<char>& contents) -> void
{

}

auto kdl::host::filesystem::file::set_bytes(const std::vector<std::uint8_t>& contents) -> void
{
...
auto kdl::host::filesystem::file::save(const filesystem::path& path) -> void
{
